...
from ..grpo_trainer_scratch.reward_func import *
...
class DAPOArguments:
    ...
    beta = 0.0            # KL散度的系数，如果为0则不使用KL散度，DAPO这里就设置为0
    clip_eps_high = 0.28
    clip_eps_low = 0.2
...
class DAPOTrainer:
    ...
    def generate_experience(self, inputs: Union[Dataset, List[Samples]]) -> List[Samples]:
        ...
        for samples in samples_list:
            ...
            with torch.no_grad():
                ...
                ###################################################################################################################
                # 把batch_XXXXX的内容全部放后边，因为可能这批数据是无效的，会跳过
    ...
    def compute_loss(self, model, inputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        coef_2 = torch.clamp(coef_1, 1 - self.args.clip_eps_low, 1 + self.args.clip_eps_high) # 上下限做了调整
        ...
        # GRPO loss
        # loss = (per_token_loss.sum(dim=1) / action_mask.sum(dim=1)) # 先在样本内token上平均loss，shape:[batch_size, num_generations, num_actions] -> [batch_size * num_generations]
        # loss = loss.mean()                                          # 再在batch上平均loss
        ...
        loss = per_token_loss.sum(-1).sum(-1) / action_mask.sum(-1).sum(-1)               # 直接在组内所有token上进行平均，shape: [batch_size]
        loss = loss.mean()
    ...
    def train(self):
        ...
        for _ in range(self.args.epoch):
            ...
            for batch in dataloader:
                ...
                # 数据组装到buffer中，可能存在没装满的batch，因此数据可能是无效的
